...
datesList.test <- list()
datesList.test[[1]] <- datesList[[1]][1:3]
datesList.test[[2]] <- datesList[[2]][1:3]
...
testfun2 <- function(x){
    ...
    writeRaster(cropped1, paste("../../media/sf_N_DRIVE/Projects/Regional/ConservationDimensions/People/TempRegulation/TopoWx/tmax", x, ".img", sep=""))
  }

system.time(lapply(datesList.test, sapply, testfun2))
...
system.time(parLapply(datesList.test, sapply, testfun2))
...
testfun <- function(x){
  for(i in length(x)){
    for(j in length(x[[i]]))
      data1 <- tmax[[which(tmax@z$Date>=x[[i]][j] & tmax@z$Date<x[[i]][j]+1)]]
    cropped1 <- crop(data1, bb1)
    cropped1 <- projectRaster(cropped1, crs="+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=23 +lon_0=-96 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m +no_defs", method='bilinear')
    writeRaster(cropped1, paste("../../media/sf_N_DRIVE/Projects/Regional/ConservationDimensions/People/TempRegulation/TopoWx/tmax", x[[i]][j], ".img", sep=""))
  }}

system.time(lapply(datesList.test, testfun))


#This works, but uses for loop instead of functionizing stuff
system.time(
  for(i in c(1:3)){
    data1 <- tmax[[which(tmax@z$Date>=dataDates[i] & tmax@z$Date<dataDates[i+1])]]
    cropped1 <- crop(data1, bb1)
    cropped1 <- projectRaster(cropped1, crs="+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=23 +lon_0=-96 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m +no_defs", method='bilinear')
    writeRaster(cropped1, paste("../../media/sf_N_DRIVE/Projects/Regional/ConservationDimensions/People/TempRegulation/TopoWx/tmax", dataDates[i], ".img", sep=""))
  })
